...
export const RegistryContractInteractions: FC = () => {
  ...
  const fetchGetHash = async () => {
    ...
    try {
      ...
      const { result, output } = await contract.query.getHash(activeAccount.address, {
        gasLimit: -1,
        storageDepositLimit,
      })

      if (result.isErr) throw new Error(result.asErr.toString())
      setGreeterMessage(output?.toString())
    } catch (e) {
      console.error(e)
      toast.error('Error while fetching get_hash. Try again…')
